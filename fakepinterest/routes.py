...
def perfil(id_usuario):
    if int(id_usuario) == int(current_user.id):
        ...
        if form_foto.validate_on_submit():
            ...
            # salvar arquivo no fotos_post
            caminho = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                               app.config["UPLOAD_FOLDER"], nome_seguro)
            arquivo.save(caminho)
            # registra banco de dados
...
@app.route("/feed")
 # CAMINHO PARA A PAG FEED
@login_required
# NECESSARIO ESTAR LOGADO PARA VER A BARRA FEED
def feed():
    fotos= Foto.query.order_by(Foto.data_criacao).all()
    #MOSTRA AS FOTOS NO FEED
    return render_template("feed.html", fotos=fotos)
    #RETORNA A PAGINA FEED
